...
export class MissionthemeComponent implements OnInit {
  ...
  //         setTimeout(() => {
  //           // this.router.navigate(['/']); // navigate to some other route
  ...
  DeleteMissionTheme(): void {
    this.service.DeleteMissionTheme(this.themeId).subscribe(
      (data: any) => {
        if (data.result === 1) {
          this.toast.success({ detail: 'SUCCESS', summary: data.data, duration: 3000 });
          this.CloseRemoveMissionThemeModal();
          // Update the missionThemeList directly
          this.missionThemeList = this.missionThemeList.filter(theme => theme.id !== this.themeId);
        } else {
          this.toast.error({ summary: data.message, duration: 3000 });
        }
      },
